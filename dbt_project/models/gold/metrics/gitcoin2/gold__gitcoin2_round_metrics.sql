...
SELECT
    ...
    COALESCE(gp."co.gitcoin.matchAmount"::numeric, 0) as match_amount_raw,
    COALESCE(gp."co.gitcoin.totalDistributed"::numeric, 0) as total_distributed_usd,

    -- Application counts per round
    ...
    CASE
        ...
        THEN ROUND((app_stats.approved_applications::numeric / app_stats.total_applications) * 100, 1)
        ELSE 0
...
LEFT JOIN (
    ...
    GROUP BY "grantPoolId"
) app_stats ON gp.id = app_stats.grant_pool_id
