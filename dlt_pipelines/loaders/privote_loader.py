...
DATABASE_URL = os.getenv(
    "DATABASE_URL", "postgresql+psycopg2://postgres:postgres@postgres:5432/opengrants"
)

RPC_ENDPOINT = os.getenv(
    "RPC_ENDPOINT", "https://arb-mainnet.g.alchemy.com/v2/YOUR_KEY"
)
CONTRACT_ADDRESS = os.getenv(
    "PRIVOTE_CONTRACT", "0xF13833531B5018C1Cb93C01100b6A9BEd1c12De8"
)
                            ...
                            record.update(
                                {
                                    ...
                                    "profile_image_url": metadata.get(
                                        "profile_image_url"
                                    ),
                                    "banner_image_url": metadata.get(
                                        "banner_image_url"
                                    ),
                                    ...
                                    "contribution_description": metadata.get(
                                        "contribution_description"
                                    ),
                                    "impact_description": metadata.get(
                                        "impact_description"
                                    ),
                                    "submitted_at": metadata.get("submitted_at"),
                                    "creator": metadata.get("creator"),
                                    # Keep nested structures as JSON
                                    "contribution_links": metadata.get(
                                        "contribution_links"
                                    ),
...
def run_pipeline():
    ...
    print(
        f"✅ Loaded {len(completed)} Privote records into Postgres → schema: {dataset}"
    )
