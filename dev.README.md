...
opengrants-platform/
├── dlt_pipelines/      → Extract & Load (DLT)
├── dbt_project/        → Transform (DBT)
├── og_dagster/            → Orchestration (Dagster)
├── nextjs-dashboard/   → Frontend (Next.js)
├── duckdb/data/        → Gold Layer storage
...
```
      ┌──────────┐
      │  Bronze  │  ← raw API / CSV / Airtable data
      └────┬─────┘
           │
           ▼
      ┌──────────┐
      │  Silver  │  ← cleaned + normalized DAOIP-5 schema
      └────┬─────┘
           │
           ▼
      ┌──────────┐
...
* For local testing without Docker, you can `source .venv/bin/activate` and run scripts directly.
* Dagster can be started manually with:

  ```bash
  dagster dev -f og_dagster/jobs/etl_job.py
  ```
* DLT pipelines can be triggered via:

  ```bash
  python dlt_pipelines/loaders/sample_loader.py
  ```
